...
-- In this case, the inner query is a summary query and the outer query is not,
...
-- It removes unused table, redundant inner and outer joins may be removed
-- TF 3604 enables output in the messages page
-- TF 8606 enables the output of the parse tree in the different phases of optimization
SELECT
  P.StockItemID
  ,P.StockItemName
FROM
...
-- Contradictions
-- Query Optimizer detects contradictions, such as opposite conditions in the WHERE clause
...
-- Why the Query Optimizer, in this case, can not see the contradictions?
-- We shouldn't need to access the table

-- Let's see the execution plan, it's parameterized and it's trivial

-- According to official Microsoft documentation, simple parameterization
-- can only occurs if the plan is a trivial plan. Again, simple parameterization
-- is considered when the optimizer thinks that the predicate changing would
-- not have a significative effect on the query plan
