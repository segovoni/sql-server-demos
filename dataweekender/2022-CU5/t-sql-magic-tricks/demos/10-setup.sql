...
RESTORE DATABASE [WideWorldImportersDW]
  FROM DISK = N'C:\SQL\DBs\Backup\WideWorldImportersDW-Full.bak' WITH FILE = 1
  ,MOVE N'WWI_Primary' TO N'C:\SQL\DBs\WideWorldImportersDW.mdf'
  ,MOVE N'WWI_UserData' TO N'C:\SQL\DBs\WideWorldImportersDW_UserData.ndf'
  ,MOVE N'WWI_Log' TO N'C:\SQL\DBs\WideWorldImportersDW.ldf'
  ,MOVE N'WWIDW_InMemory_Data_1' TO N'C:\SQL\DBs\WideWorldImportersDW_InMemory_Data_1'
  ,NOUNLOAD
  ,STATS = 5;
GO
*/
...
------------------------------------------------------------------------
DROP INDEX [CCX_Fact_Movement] ON [Fact].[Movement];
ALTER TABLE [Fact].[Movement] DROP CONSTRAINT [PK_Fact_Movement];
ALTER TABLE [Fact].[Movement] ADD CONSTRAINT [PK_Fact_Movement] PRIMARY KEY CLUSTERED 
(
	[Movement Key] ASC,
	[Date Key] ASC
);
...
CREATE NONCLUSTERED INDEX IX_Fact_Movement_Stock_Item_Key
  ON [Fact].[Movement]
  (
    [Stock Item Key]
    ,[Movement Key]
